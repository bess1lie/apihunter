...
### Fixed
- CORS: Real active probing using `AnalyzerContext.executor`
- RateLimit: 5-probe heuristic using budget-aware executor; safe fallback
- Injection: Safe active `'` probe with multi-sign SQL error fragment validation
- Auth: Robust 2xx-if-spec-says-auth heuristic
- IDOR: Refined heuristic targeting object-like params
- Discovery: Resilience (provider failure), malformed spec parsing (null/invalid path)
- Reporting: SARIF `artifactLocation` URI precision

### Added
- Test: Behavioral API (`tests/test_behavioral_api.py`) with 11 integration test cases
- Chore: Global request budget enforcement in `Executor`
